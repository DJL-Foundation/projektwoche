...
impl DownloadAndExec {
  fn new(url: &'static str, filetype: Filetype, silent: bool, custom_args: Option<&'static [&'static str]>) -> Self {
    Self { url, filetype, silent, custom_args }
  }
}
...
impl AnyInstruction for ExtractArchive {
  fn run(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let extension = path.extension().and_then(|s| s.to_str()).ok_or("No file extension")?;
...
impl WaitForCondition {
  fn new(check_command: &str, timeout_secs: u64) -> Self {
    Self {
      check_command: check_command.split_whitespace().map(|s| s.to_string()).collect(),
      timeout_secs,
    }
  }
}
...
impl AnyInstruction for InstallPackage {
  fn run(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let package_managers = [
      ...
      ("pacman", vec!["pacman", "-S", "--noconfirm", self.package_name]),
...
impl AnyInstruction for RestartService {
  fn run(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    #[cfg(unix)]
    {
      if Command::new("systemctl").arg("--version").output().map(|o| o.status.success()).unwrap_or(false) {
        Command::new("systemctl")
          .args(&["restart", self.service_name])
          .status()?;
      } else if Command::new("service").arg("--version").output().map(|o| o.status.success()).unwrap_or(false) {
        Command::new("service")
...
impl Instruction {
  ...
  pub fn download_and_exec(mut self, filetype: Filetype, url: &'static str) -> Self {
    self.instruction = Some(Instructions::DownloadAndExec(DownloadAndExec::new(url, filetype, false, None)));
    self
  }
  
  pub fn download_and_exec_silent(mut self, filetype: Filetype, url: &'static str) -> Self {
    self.instruction = Some(Instructions::DownloadAndExec(DownloadAndExec::new(url, filetype, true, None)));
    self
  }
  
  pub fn download_and_exec_with_args(mut self, filetype: Filetype, url: &'static str, args: &'static [&'static str]) -> Self {
    self.instruction = Some(Instructions::DownloadAndExec(DownloadAndExec::new(url, filetype, false, Some(args))));
    self
  }
  ...
  pub fn download_to(mut self, url: &'static str, path: &'static str) -> Self {
    self.instruction = Some(Instructions::DownloadTo(DownloadTo::new(url, path)));
    self
  }
  
  pub fn check(mut self, command: &str, expect: &'static str) -> Self {
    self.instruction = Some(Instructions::Assert(Assert::new(command, expect)));
    self
  }
  
  pub fn extract_archive(mut self, archive_path: &'static str, destination: &'static str) -> Self {
    self.instruction = Some(Instructions::ExtractArchive(ExtractArchive::new(archive_path, destination)));
    self
  }
  
  pub fn add_env_var(mut self, name: &'static str, value: &'static str) -> Self {
    self.instruction = Some(Instructions::AddEnvVar(AddEnvVar::new(name, value)));
    self
  }
  
  pub fn create_shortcut(mut self, name: &'static str, target: &'static str, icon: Option<&'static str>) -> Self {
    self.instruction = Some(Instructions::CreateShortcut(CreateShortcut::new(name, target, icon)));
    self
  }
  
  pub fn wait_for_condition(mut self, check_command: &str, timeout_secs: u64) -> Self {
    self.instruction = Some(Instructions::WaitForCondition(WaitForCondition::new(check_command, timeout_secs)));
    self
  }
  
  pub fn install_package(mut self, package_name: &'static str) -> Self {
    self.instruction = Some(Instructions::InstallPackage(InstallPackage::new(package_name)));
    self
  }
  
  pub fn clone_repository(mut self, url: &'static str, path: Option<&'static str>) -> Self {
    self.instruction = Some(Instructions::CloneRepository(CloneRepository::new(url, path)));
    self
  }
  
  pub fn request_sudo(mut self, reason: &'static str) -> Self {
    self.instruction = Some(Instructions::RequestSudo(RequestSudo::new(reason)));
    self
  }
  
  pub fn restart_service(mut self, service_name: &'static str) -> Self {
    self.instruction = Some(Instructions::RestartService(RestartService::new(service_name)));
    self
  }
  
  pub fn backup_file(mut self, path: &'static str) -> Self {
    self.instruction = Some(Instructions::BackupFile(BackupFile::new(path)));
    self
  }
  
  pub fn edit_file(mut self, path: &'static str, find: &'static str, replace: &'static str) -> Self {
    self.instruction = Some(Instructions::EditFile(EditFile::new(path, find, replace)));
    self
  }
  
  pub fn add_to_path(mut self, path: &'static str) -> Self {
